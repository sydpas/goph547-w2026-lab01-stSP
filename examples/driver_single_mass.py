...
def main():
    ...
    U_25 = np.zeros((x_25.shape[0], x_25.shape[1], len(zp)))  # store grav pot at each grid point for each height
    g_25 = np.zeros((x_25.shape[0], x_25.shape[1], len(zp)))

    U_5 = np.zeros((x_5.shape[0], x_5.shape[1], len(zp)))  # same but for 5
    ...
    for k, z_val in enumerate(zp):
        for i in range(x_25.shape[0]):
            for j in range(x_25.shape[1]):
                ...
                U_25[i,j,k] = gravity_potential_point(x, xm, m, G=6.674e-11)

    for k, z_val in enumerate(zp):
        for i in range(x_25.shape[0]):
            for j in range(x_25.shape[1]):
                x= np.array([x_25[i, j], y_25[i, j], z_val])
                g_25[i,j,k] = gravity_effect_point(x, xm, m, G=6.674e-11)

    for k, z_val in enumerate(zp):
        for i in range(x_5.shape[0]):
            for j in range(x_5.shape[1]):
                x= np.array([x_5[i, j], y_5[i, j], z_val])
                U_5[i,j,k] = gravity_potential_point(x, xm, m, G=6.674e-11)

    for k, z_val in enumerate(zp):
        for i in range(x_5.shape[0]):
            for j in range(x_5.shape[1]):
                x= np.array([x_5[i, j], y_5[i, j], z_val])
                g_5[i,j,k] = gravity_effect_point(x, xm, m, G=6.674e-11)

    #  out of the loop so they don't reset
    Umin, Umax = np.min(U_25), np.max(U_25)
    gmin, gmax = np.min(g_25), np.max(g_25)
    ...
    for k, z_val in enumerate(zp):
        # U plot (left column)
        ax = axes[k, 0]
        U_cf_25 = ax.contourf(x_25, y_25, U_25[:,:,k], levels=U_levels, cmap='viridis', vmin=Umin, vmax=Umax)
        ...
        # g plot (right column)
        ax1 = axes[k, 1]
        g_cf_25 = ax1.contourf(x_25, y_25, g_25[:,:,k], levels=g_levels, cmap='plasma', vmin=gmin, vmax=gmax)
    ...
    Umin, Umax = np.min(U_5), np.max(U_5)
    gmin, gmax = np.min(g_5), np.max(g_5)
    ...
    for k, z_val in enumerate(zp):
        # U plot (left column)
        ax = axes[k, 0]
        U_cf_5 = ax.contourf(x_5, y_5, U_5[:,:,k], levels=U_levels, cmap='viridis', vmin=Umin, vmax=Umax)
        ...
        # g plot (right column)
        ax1 = axes[k, 1]
        g_cf_5 = ax1.contourf(x_5, y_5, g_5[:,:,k], levels=g_levels, cmap='plasma', vmin=gmin, vmax=gmax)
